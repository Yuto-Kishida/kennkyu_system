var mp3explosion = 'data:audio/mp3;base64, ';

var prefixes = ["webkit", "moz", "ms", ""];
function prefixedEvent(element, type, callback) {
	for (var p = 0; p < prefixes.length; p++) {
		if (!prefixes[p]) type = type.toLowerCase();
		element.addEventListener(prefixes[p]+type, callback, false);
	}
}

//位置移動
function transform ($e, x, y, scale, rotation, percent) {
  x = x || 0; y = y || 0; scale = scale || 1;
  unit = percent ? '%' : 'px';
  rotation = rotation || 0;
  
  transfromString = 'translate('+ x + unit + ', '+ y + unit + ') ' 
  								+ 'scale(' + scale + ') '
  								+ 'rotate(' + rotation + 'deg)';
  
  $e.style.webkitTransform = transfromString;
  $e.style.MozTransform = transfromString;
  $e.style.transform = transfromString;
}


//パーティクルの作成
function createParticle(x, y, scale) {
  var $particle = document.createElement('i');
 	var $sparcle = document.createElement('i');
  
  $particle.className = 'particle';
  $sparcle.className = 'sparcle';
  
  transform($particle, x, y, scale);
  $particle.appendChild( $sparcle );
  
  return $particle;
}


function explode ($container) {
  var particles = [];
  let plus = 0.2

  particles.push( createParticle(0,0,1+plus) );
  particles.push( createParticle(70,-35,0.4+plus) );
  particles.push( createParticle(70,-125,0.2+plus) );
  particles.push( createParticle(-30,-80,0.8+plus) );
  particles.push( createParticle(-30,80,0.4+plus) );
  particles.push( createParticle(-70,-80,0.2+plus) );
  particles.push( createParticle(-70,-35,0.75+plus) );
  particles.push( createParticle(-120,-26,0.4+plus) );
  particles.push( createParticle(-120,-35,0.2+plus) );
  particles.push( createParticle(-120,-135,0.2+plus) );
  particles.push( createParticle(100,-35,0.1+plus) );

	particles.forEach(function(particle){
    $container.appendChild( particle );
    prefixedEvent(particle, "AnimationEnd", function(){
      var self = this;
      setTimeout(function(){
        requestAnimationFrame(function(){
          $container.removeChild(self);         
        });
     	},100);
    });
  });
  
}

//containerの作成：
function exolpodeGroup (x, y, trans) {
  var $container = document.createElement('div');
  
  $container.className = 'container';
  $container.style.top = y + 'px';
  $container.style.left = x + 'px';
  
  transform( $container, trans.x, trans.y, trans.scale, trans.r, true );
  
  explode( $container );
	return $container;
}


function sparcle(area,area2) {
  let textScreen = document.getElementsByClassName("text");
  let explosions = [];
  let destoryFlag = 0;

  if(textScreen.length){
    console.log(textScreen)
    Array.prototype.forEach.call(textScreen,element => {
      var rect = element.getBoundingClientRect();
      console.log(rect.left)
      if(area  <= rect.left && rect.left <= area2){
        explosions.push( exolpodeGroup(rect.left, rect.top, {scale: 1, x: -50, y: -50, r: 0}) );
        explosions.push( exolpodeGroup(rect.left, rect.top, {scale: .5, x: -30, y: -50, r: 180}) );
        explosions.push( exolpodeGroup(rect.left, rect.top, {scale: .5, x: -50, y: -20, r: -90}) );
        element.parentNode.removeChild(element);
        destoryFlag = 1
      }
    });

    if(destoryFlag === 1){
      var audio = new Audio();
      audio.src = mp3explosion;
      
      requestAnimationFrame(function(){
        audio.play();
        explosions.forEach(function(boum, i){
          setTimeout(function(){
            document.body.appendChild( boum );
            setTimeout(function(){
              document.body.removeChild( boum )
            },1000)
          }, i * 100);
        });
      });
    }
  }
}


//クリック時の動作
document.addEventListener("keydown", event => {
  let windowWidth = window.innerWidth;
  let rightLine =  windowWidth / 3
  let centerLine = rightLine * 2;

 
  if (event.key === "b"){
    sparcle(0,rightLine);
  }else if(event.key === "PageDown"){
    sparcle(rightLine,centerLine);
  }else if(event.code === "PageUp"){
    sparcle(centerLine,windowWidth);
  }
});


// ソースコードの説明
// １：クリック時にsparcle関数を動作
// ２：3つのexolpodeGroup関数を動作
//    containerクラスのdivを作成する
//    exolpodeGroupの位置を決定する
//    
// 
